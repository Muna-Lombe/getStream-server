    ...
    do 
        ...
        if [ ${BASH_VERSION%%[^0-9]*} -lt 4 ]
            then
                ...
                name="app$id"
                id=`readJson ./trial_extender/collected/app$id.json id` || exit 1;
                appkey=`readJson ./trial_extender/collected/app$id.json key` || exit 1;
                appsec=`readJson ./trial_extender/collected/app$id.json secret` || exit 1;
            else
                echo "using bash ${BASH_VERSION%%[^0-9.]*}"
                # if [ -e "./trial_extender/collected/app$id.json" ]
                #     then
                #         echo "File exists"
                #     else
    ...
    then
        ...
        then
            ...
            then
                ...
                id=`readJson ./trial_extender/streamData.json id` || exit 1;
                appkey=`readJson ./trial_extender/streamData.json appkey` || exit 1;
                appsec=`readJson ./trial_extender/streamData.json appsec` || exit 1;
            else
                echo ${BASH_VERSION%%[^0-9.]*}
                id=`jq -r  '.id' ./trial_extender/streamData.json` 
                appkey=`jq -r '.appkey' ./trial_extender/streamData.json` 
                appsec=`jq -r '.appsec' ./trial_extender/streamData.json`
            fi

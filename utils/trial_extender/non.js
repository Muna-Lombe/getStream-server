...
try {
  ...
  if (!process.cwd().includes("/trial_extender")) {
    console.log("path is set correctly", process.cwd());
...
function setEnvValue(key, value) {
  ...
  const ENV_VARS = fs.readFileSync(basepath+"/.env", "utf8").split(os.EOL);

  console.log(".env current", ENV_VARS);
  ...
  fs.writeFileSync(basepath+"/.env", ENV_VARS.join(os.EOL));
...
async function fs_do(type, path, file, data, write_new=false, id=0) {
  ...
  if (type==="write") {
    if (write_new === true) {
      return fs.writeFile(`${basepath}/utils/trial_extender/collected/app1.json`, data, (err) => {
        if (err) throw err;
...
async function cleanSlate(params) {
  ...
  const paths = [
        `${basepath}/utils/trial_extender/collected/app1.json`,
        `${basepath}/utils/trial_extender/cleanedResponseHeader.json`,
        `${basepath}/utils/trial_extender/collectiveStreamData.json`,
        `${basepath}/utils/trial_extender/new_app_data.json`,
        `${basepath}/utils/trial_extender/streamCred.json`,
        `${basepath}/utils/trial_extender/streamData.json`,
        `${basepath}/utils/trial_extender/streamData.txt`
  ]
  try {
    ...
    console.log("file truncate failed", error);
    return false;
  }
}
...
async function signup(timeout=1000) {
  ...
  let checkFile;
  try {
    checkFile = fs.readFileSync(`${basepath}/utils/trial_extender/streamCred.json`);
  } catch (error) {
    fs.writeFile(`${basepath}/utils/trial_extender/streamCred.json`, JSON.stringify([{"username": "TeamTay", "email": "lombemuna@hotmail.com", "first_name": "", "last_name": ""}]), (err) => {
      if (err) throw err;
  ...
  fs.writeFileSync(`${basepath}/utils/trial_extender/streamCred.json`, JSON.stringify(em_res));
...
function setCookies(header) {
  ...
  try {
    ...
    fs.writeFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`, JSON.stringify(finalCookies));
...
async function login(counter = 0) {
  ...
  async function fetchUserCred() {
    try {
      // await JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/streamCred.json`).toString())
      const streamCred = await JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/streamCred.json`).toString());
  ...
  fs.writeFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`, JSON.stringify({csrftoken: newCsrf.slice(newCsrf.indexOf("=")+1, newCsrf.length), sessionid: newSessionid.slice(newSessionid.indexOf("=")+1, newSessionid.length)}));
  return 1;
  ...
  // try {
  //   fs.truncate(`${basepath}/utils/trial_extender/stringHeaderResponse.txt`,0, function(){console.log('String Header truncated')})
  // } catch (error) {
  //   console.log('stringHeaderResponse does not exist')
  //   fs.writeFile(`${basepath}/utils/trial_extender/stringHeaderResponse.txt`,JSON.stringify(lr.headers), (err) => {
  //     if (err) throw err;
...
async function createApp(app_count=0) {
  ...
  const {id, csrftoken, sessionid, user_token} = await JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`).toString()); // JSON.parse(JSON.stringify(fs_do('read',basepath,'cleanedResponseHeader.json')))
  const {username} = await JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/streamCred.json`).toString());
  ...
  fs_do("write", basepath, "utils/trial_extender/new_app_data.json", JSON.stringify(data));
  ...
  try {
    ({appid, key, secret}=await JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/collected/${appname}.json`).toString()));
  } catch (err) {
  ...
  fs_do("write", basepath, "utils/trial_extender/collectiveStreamData.json", JSON.stringify(apps));
...
async function getOptions() {
  ...
  const {id, csrftoken, sessionid, user_id, user_token} = fs.readFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`).toString() ? await JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`).toString()) : {}; // JSON.parse(JSON.stringify(fs_do('read',basepath,'cleanedResponseHeader.json')))
  const baseurl = `https://getstream.io/api/dashboard/organization/${id}/app/`;
  ...
  if (gor.status.code === 200) {
    ...
    fs.writeFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`, JSON.stringify(newCleanedResponseHeaders));
  }
  return 0;
}
...
async function genApp(mode="default state", command="continue") {
  ...
  // JSON.parse(fs.readFileSync(`${basepath}/utils/trial_extender/cleanedResponseHeader.json`).toString())
  ...
  if ((typeof id) === "undefined" || (typeof user_token) === "undefined" || (typeof sessionid) === "undefined" || (typeof csrftoken )=== "undefined" || command === "new_cred") {
    ...
    fs_do("write", basepath, "utils/trial_extender/streamData.txt", cookieString);

    fs_do("write", basepath, "utils/trial_extender/streamData.json", JSON.stringify({org_id, user_id, user_token, appkey, appsec}));

    fs_do("write", basepath, "utils/trial_extender/cleanedResponseHeader.json", JSON.stringify({id: org_id, csrftoken, sessionid, user_id, user_token}));
